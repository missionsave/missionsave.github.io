...
void WriteBinarySTL(const TopoDS_Shape& shape, const std::string& filename)
{
    ...
    for (TopExp_Explorer faceExp(shape, TopAbs_FACE); faceExp.More(); faceExp.Next()) {
        ...
        TopLoc_Location loc = face.Location();   // make a non-const copy
Handle(Poly_Triangulation) tri = BRep_Tool::Triangulation(face, loc);
    ...
    for (TopExp_Explorer faceExp(shape, TopAbs_FACE); faceExp.More(); faceExp.Next()) {
        ...
        for (int i = tris.Lower(); i <= tris.Upper(); ++i) {
            ...
            float n[3] = { (float)normal.X(), (float)normal.Y(), (float)normal.Z() };
            ...
            file.write((char*)coords, 36);

            uint16_t abc = 0;
            file.write((char*)&abc, 2);
        }
    }
